...
if __name__ == '__main__':
    ...
    ethernet = Ether(dst='00:50:56:ed:b4:48', src=hw, type=0x800)
    ip = IP(src='192.168.128.12', dst='192.168.128.254')
    udp = UDP(sport=68, dport=67)
    bootp = BOOTP(xid=client_xid, flags=0, op=1, giaddr='192.168.128.12', chaddr=hw)
    dhcp = DHCP(options=[("message-type", "discover"), ("hostname", "server-01"), ('end')])
    packet = ethernet / ip / udp / bootp / dhcp

    sendp(packet, iface=conf.iface)

    sniff(prn=on_packet, store=0, count=1, timeout=10, iface=conf.iface, filter="udp and src port 67 and dst port 67")
